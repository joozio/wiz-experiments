...
function LiveCounter({ stat, startValue }: { stat: StatCategory; startValue: number }) {
  ...
  // startValue lands as 0 during the pre-mount render and only gets its real figure
  // once the parent has read the client clock, so adopt it when it arrives.
  useEffect(() => {
    setCount(startValue);
  }, [startValue]);
...
function ClockDisplay() {
  // The site is a static export, so the HTML here is frozen at build time. Reading the
  // real clock during the first client render can therefore never match it, and React
  // throws hydration error #418. The clock stays a placeholder until mount instead of
  // rendering a build-time reading that would visibly jump to the correct one.
...
// Both figures count from the visitor's local midnight, so they can only be computed
// once we are in the browser. Doing it in a state initializer ran it at build time on
// the server pass and again with a different clock on the client, which is the second
// half of this page's hydration mismatch.
function computeDayTotals() {
  ...
  return { totalActions: total as number | null, startValues: values };
}

const ZERO_TOTALS = {
  totalActions: null as number | null,
  startValues: STAT_CATEGORIES.reduce((acc, stat) => {
    acc[stat.id] = 0;
    return acc;
  }, {} as Record<string, number>),
};
...
export default function AutomationClock() {
  const [{ totalActions, startValues }, setDayTotals] = useState(ZERO_TOTALS);

  useEffect(() => {
    setDayTotals(computeDayTotals());
  }, []);
