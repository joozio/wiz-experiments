...
export default function ModelBlindfoldPage() {
  ...
  // Safe only because `phase` starts at 'intro' and nothing derived from this seed is
  // rendered until the visitor reaches 'challenge'. The site is a static export, so a
  // random value that reaches the FIRST render fails hydration on every visit (React
  // #418) the way automation-clock and social-grid did. If 'challenge' ever becomes the
  // initial phase, seed this from a build timestamp passed in by the server component.
